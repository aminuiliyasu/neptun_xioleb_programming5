...
class GameService {
  ...
  async makeMove(userId, gameId, move) {
    ...
    if (playersWithMoves === game.players.length) {
      ...
      // Notify players of move made - this tells the next player it's their turn
      console.log(`[GameService] Player ${userId.substring(0, 8)} made move: ${move}, next player: ${game.currentPlayer.substring(0, 8)}`);
      this.broadcastToPlayers(game.players, {
        type: 'move_made',
        data: {
          gameId,
          player: userId,
          move: move, // Don't reveal the move yet
          currentPlayer: game.currentPlayer // This is the player who should move next
        }
      });
    }
  ...
  compareMoves(move1, move2) {
    ...
    if (winConditions[move1] === move2) {
      ...
      return { winner: 'player2', result: GameResult.WIN };
    }
  }
  ...
  broadcastToPlayers(playerIds, message) {
    console.log(`[GameService] Broadcasting ${message.type} to ${playerIds.length} players`);
    let sentCount = 0;
    playerIds.forEach(playerId => {
      ...
      if (client && client.readyState === WebSocket.OPEN) {
        try {
          ...
          sentCount++;
          console.log(`[GameService] Sent ${message.type} to player ${playerId.substring(0, 8)}...`);
        } catch (error) {
          console.error(`[GameService] ✗ Error sending to player ${playerId}:`, error.message);
        }
      } else {
        const state = client ? client.readyState : 'no client';
        console.log(`[GameService] ✗ Player ${playerId.substring(0, 8)}... not connected (state: ${state})`);
      }
    });
    console.log(`[GameService] Successfully sent to ${sentCount}/${playerIds.length} players`);
  }
...
wss.on('connection', (ws, req) => {
  try {
    ...
    if (!userId) {
      console.error('WebSocket connection rejected: User ID required');
